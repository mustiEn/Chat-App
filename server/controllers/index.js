...
import { logger, sendMail } from "../utils/constants.js";
import { validationResult, matchedData } from "express-validator";
import { User } from "../models/Users.js";
import { SignupRequest } from "../models/SignupRequests.js";
...
const login = async (req, res, next) => {
  try {
    ...
    logger.log(user.toJSON().username);
    if (!user) {
      throw new Error("User not found");
    }
    logger.log("SESSION AND ID AT LOGIN ==========", req.session.passport.user);
    res.status(200).json({
      message: "Signup",
    });
  } catch (error) {
